from entities.entity import Entity
from systems.rng_system import RngSystem, Generator
import utils.math

import pygame
import numpy as np
from noise import pnoise1

class XpEntity(Entity):
    def __init__(self, initial_pos, player):
        image = pygame.Surface((8, 8))
        image.fill((0, 0, 255))
        super().__init__(image, initial_pos)

        self.velocity = np.zeros(2)
        self.player = player
        self.timer = 0

    def setup(self):
        super().setup()
        rng = RngSystem().get_rng(Generator.ENEMIES)
        self.velocity = utils.math.rotate_vector(np.array((1.0, 0.0)), 180.0 * rng.random()) * 10.0

    def update(self, elapsed_time):
        player_x, player_y = self.player.get_position()
        direction = np.array((player_x - self.x, player_y - self.y))
        distance = np.linalg.norm(direction)

        if distance < 20:
            self.player.increase_exp(10)
            self.kill()
            return
        
        self.timer += elapsed_time
        self.move((pnoise1(self.timer / 1000.0) * 2.0, pnoise1(self.timer / 1000.0 + 20711.0) * 2.0))

        self.velocity += (direction * 25.0 / distance) / distance**2 * elapsed_time

        self.move(self.velocity)

        super().update(elapsed_time)
